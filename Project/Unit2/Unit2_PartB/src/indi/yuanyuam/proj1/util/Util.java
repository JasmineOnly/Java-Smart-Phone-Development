					...
					for (int j = 0; j < optionInfo.length; j++) {
						...
						try {
							...
							if (optionDetail.length != 2) {
								throw new AutoException(ExceptionEnum.FileMissOptionPrice);
							}
						} catch (AutoException e) {
							Helper helper = new Helper();
							optionDetail[1] = Integer.toString(helper.fixFileMissOptionPrice());
						}
						
						auto.setOption(optionSetName,optionDetail[0],Integer.parseInt(optionDetail[1]));
					}
				}
			}
